...
x_step_friis = total_range / num_friis_points; %mm
...
pause(1);
fopen(MXAObj);
% fprintf(MXAObj, 'SENSe:FREQuency:CENTer 1e9');
% fprintf(MXAObj, 'SENSe:FREQuency:SPAN 1e9');
% fprintf(MXAObj, 'SENSe:BANDwidth:RESolution 8e6');
fprintf(MXAObj, 'SENSe:FREQuency:CENTer 616e9');
fprintf(MXAObj, 'SENSe:FREQuency:SPAN 10e9');
fprintf(MXAObj, 'SENSe:BANDwidth:RESolution 8e6');
%% Start Measurements
peak_amp = -100 .* ones(num_friis_points, 2*num_rail_points);

figure;

N = floor(x_step_friis / x_step);

for i=1:num_friis_points
...
filename = sprintf('TX_616_GHZ_TEST_H_PLANE_FRIIS_DATE_%s.csv', timestamp);
csvwrite(filename, peak_amp);
%% Plotting The Pattern
figure;
% theta = -total_angle/2:step_size_deg:total_angle/2-step_size_deg;
x = 30:x_step_friis:280 - x_step_friis;
semilogx(x, max(peak_amp, [], 2), 'LineWidth', 2);
grid on;
hold on;
semilogx(x, mean(peak_amp, 2), 'LineWidth', 2);
% delete(stage);
% clear stage;
clc;