...
FILENAME = 'GlucoseWaterConcentration.csv'
PLOT_TITLE = '5-Slide Glucose in Water Absorption/Reflection Analysis'
...
try:
    ...
    df['Concentration(M)'] = pd.to_numeric(df['Concentration(M)'], errors='coerce')
    ...
    for sample_name, group_data in df.groupby('Sample'):
        plt.plot(group_data['Concentration(M)'], group_data['Power (dBm)'], marker='o', linestyle='-', label=sample_name)
