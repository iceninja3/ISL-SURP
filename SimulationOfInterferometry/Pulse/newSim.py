...
bpm = 80 # Beats per minute for the pulse signal
...
# --- NEW: Generate a heartbeat-like pulse train ---
def get_heartbeat_signal(time_array, bpm):
    """Generates a periodic heartbeat signal."""
    beat_period = 60.0 / bpm
    # Time within each beat (from 0 to beat_period)
    t_local = time_array % beat_period
    
    # Create a simplified pulse using two Gaussian functions
    # A sharp, primary pulse (QRS complex)
    pulse1 = np.exp(-((t_local - 0.15 * beat_period)**2) / (2 * (0.03 * beat_period)**2))
    # A smaller, wider secondary pulse (T-wave)
    pulse2 = 0.4 * np.exp(-((t_local - 0.35 * beat_period)**2) / (2 * (0.06 * beat_period)**2))
    
    # Combine and normalize
    raw_signal = pulse1 + pulse2
    return raw_signal / np.max(raw_signal) # Normalize to have a max amplitude of 1
...
# Find the peak frequency (the fundamental frequency)
fundamental_freq_hz = bpm / 60.0
# Find the actual peak in the FFT data, which should be close to the fundamental
...
plt.grid(True)
# Add a vertical line and text to mark the fundamental frequency
plt.axvline(peak_frequency, color='r', linestyle='--', label=f'Peak (Fundamental): {peak_frequency:.2f} Hz')
plt.text(peak_frequency + 5, peak_amplitude * 0.9, f'{peak_frequency:.2f} Hz', color='r')
plt.legend()
# Limit x-axis to see the fundamental and first few harmonics
plt.xlim(0, fundamental_freq_hz * 10) # Show up to the 10th harmonic
